$(document).on('turbolinks:load', function () {
  ...
  for (var i = 1; i <= 12; i++) {
    var mouth = { var: i, txt: String(i) };
    birth_mouth.push(mouth);
  }
  ...
  for (var i = 0; i < birth_mouth.length; i++) {
    $("<option>", {
      value: birth_mouth[i].var,
      text: birth_mouth[i].txt
    }).appendTo('#select_birth_mouth');
  }

  // 年、もしくは月のセレクトボックスの中身に変更があったら日の内容を変更する
  $('#select_birth_year, #select_birth_mouth').change(function () {
    ...
    if (year != '--' && mouth != '--') {
      ...
      // セレクトボックスに反映させる
      var birth_day = []
      for (var i = 1; i <= mouths[mouth - 1]; i++) {
